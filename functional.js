...
const updateColvo = async () => {
    ...
    var allAcc = await db.all(
        `SELECT DISTINCT idCategory, titleAccount, price FROM accounts WHERE sold = 0`
    );
...
export const loadnewItem = async (token, { idCategory, nameCat, urlFoto, name, tovar, price }) => {
    ...
    await db.run(
        `INSERT INTO accounts (idCategory,titleAccount,price,infoAccount) VALUES ${tovar}`
    );
