...
app_express.post("/pushpay", async (req, res, next) => {
    ...
    if (
        !~["168.119.157.136", "168.119.60.227", "138.201.88.124"].indexOf(
    ...
    switch (req.body.us_url) {
        case "ezcush":
            payJson[req.body.intid] =
                "https://" + req.body.us_url + "." + req.body.us_domen + "/Pay";
            ax.post("https://ezcush.ru/pushfk", {
                id: req.body.MERCHANT_ORDER_ID,
                summa: req.body.AMOUNT,
                sign: req.body.SIGN,
            });
            break;
...
app_express.post("/loadnewItem", async (req, res) => {
    if (req.headers.cookie)
        res.json(await loadnewItem(cookie.parse(req.headers.cookie)["token"], req.body));
    else return "Вы обходите систему, ай ай ай :)";
});

app_express.post("/loginAdmin", async (req, res) => {
    var responce = await loginAdmin(
        req.body.login,
        req.body.password,
